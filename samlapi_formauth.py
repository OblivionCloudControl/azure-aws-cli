...
def main():
  ...
  print '\n----------------------------------------------------------------'
  print 'Your new access key pair has been stored in the AWS configuration file under the {0} profile.'.format(profile_name)
...
def o365_login(session, saml_app_id, username, password):
  ...
  redir_response = session.get("https://account.activedirectory.windowsazure.com/applications/redirecttofederatedapplication.aspx?Operation=LinkedSignIn&applicationLinkName=AWS+-+OblCC+IAM&applicationId=c48a42b758704a90b10e7095ffdaffd4")
...
def start_phone_app_mfa(session, begin_auth_url, end_auth_url, process_auth_url, flow_token, ctx, canary):
  ...
  mfa_response = session.post(begin_auth_url, data=body, headers={u"Accept": u"application/json", u"Content-Type": u"application/json"})
  ...
  while True:
    ...
    mfa_end_response = session.post(end_auth_url, data=body, headers={u"Accept": u"application/json", u"Content-Type": u"application/json"})
    #print(mfa_end_response.text)
    auth_end = mfa_end_response.json()
    if auth_end["Result"] == "true":
      return finish_mfa(session=session, process_auth_url=process_auth_url, request=auth_end["Ctx"], flow_token=auth_end["FlowToken"], canary=canary, mfa_auth_method="PhoneAppNotification")
...
def start_token_mfa(session, mfa_auth_method, begin_auth_url, end_auth_url, process_auth_url, flow_token, ctx, canary):
  ...
  auth_end = mfa_end_response.json()
  if auth_end["Result"] == "true":
    return finish_mfa(session=session, process_auth_url=process_auth_url, request=auth_end["Ctx"], flow_token=auth_end["FlowToken"], canary=canary, mfa_auth_method=mfa_auth_method)
...
def finish_mfa(session, process_auth_url, request, flow_token, canary, mfa_auth_method):
  print("MFA Success")
  data = {"request": request, "flowToken": flow_token, "canary": canary, "mfaAuthMethod": mfa_auth_method, "rememberMFA": "false"}
